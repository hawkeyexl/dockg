...
It pairs with [docmeta](https://github.com/hawkeyexl/docmeta) (which powers `dockg validate`) and follows the same CLI conventions as [docevals](https://github.com/hawkeyexl/docevals). dockg's frontmatter schema is published in this repo at [`schemas/frontmatter-0.2.json`](schemas/frontmatter-0.2.json) — point any JSON Schema tool at it, e.g. `docmeta validate --schema node_modules/dockg/schemas/frontmatter-0.2.json docs/`.
...
**Naming:** the *frontmatter key* is `kg:`; the *RDF namespace prefix* is `dockg:`. The `kg` key holds the SKOS fields dockg owns, validated by the JSON Schema published in this repo (`schemas/frontmatter-0.2.json`). Docs without a `kg` key are fine — everything above still derives.
...
**Timestamps and determinism:** wall-clock time never enters the graph. With
`provenance.gitTime: true`, the build activity gets `prov:endedAtTime` from the
corpus repo's **HEAD committer date** — deterministic per commit, so rebuilds
at the same commit stay byte-identical (errors loudly outside a git repo).
...
These fields are validated by **`schemas/frontmatter-0.2.json`** (bundled with
the package; the default for `dockg validate`). The pre-provenance
`schemas/frontmatter-0.1.json` remains published alongside it.
...
| `dockg validate [globs]` | Check KG frontmatter via docmeta (bundled `schemas/frontmatter-0.2.json`) |
...
provenance:
  gitTime: false     # opt-in: stamp the build with the HEAD committer date
# validate.schemas defaults to the bundled schemas/frontmatter-0.2.json
fill:
  provider: anthropic
  temperature: 0
  maxCostUsd: 5
