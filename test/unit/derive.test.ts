...
describe("deriveGraph — provenance", () => {
  ...
  it("maps kg.provenance entries to per-model fill activities, not shared subjects", () => {
    ...
    expect(
      has(g, claudeEntry, `${NS.dockg}filledField`, lit("prefLabel")),
    ).toBe(true);
    ...
    expect(has(g, gptEntry, `${NS.dockg}filledField`, lit("subjects"))).toBe(
      true,
    );
  ...
  it("still accepts the legacy single-object kg.provenance form", () => {
    ...
    expect(has(g, activity, `${NS.dockg}filledFieldEntry`, iri(entry))).toBe(
      true,
    );
    expect(has(g, entry, `${NS.dockg}filledField`, lit("subjects"))).toBe(true);
  });
