...
describe("prompt FIELD_SCHEMAS ↔ bundled schema", () => {
  const schema = JSON.parse(
    ...
    $defs?: { provenanceEntry?: { properties?: { fields?: { items?: { enum?: string[] } } } } };
  };
  ...
  it("every fillable field exists in the bundled schema with a matching type", () => {
    for (const [field, fieldSchema] of Object.entries(FIELD_SCHEMAS)) {
      expect(kgProperties, `schema is missing fill field "${field}"`).toHaveProperty(field);
      expect(kgProperties[field]!.type).toBe((fieldSchema as { type: unknown }).type);
    }
  });

  it("the provenance fields enum covers every fillable field", () => {
    const allowed = schema.$defs?.provenanceEntry?.properties?.fields?.items?.enum ?? [];
